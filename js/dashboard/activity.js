...
export async function mount(ctx, container) {
  ...
  // ── Admin Tasks (smart to-do, very top) ───────────────────────────────────
  // A prioritized, plain-English list of exactly what the admin should do next,
  // synthesized from the same project/people signals the rest of the page uses.
  const tasksCard = el("div", { class: "card admin-tasks-card" });
  tasksCard.innerHTML = `
    <div class="card-header">
      <div>
        ...
        <div class="card-subtitle">What needs your attention right now — in priority order. Updates live.</div>
      </div>
      <div style="display:flex;gap:8px;align-items:center;">
        <span class="admin-tasks-count" id="admin-tasks-count"></span>
      </div>
    </div>
    <div class="card-body" id="admin-tasks-body">
      <div class="loading-state"><div class="spinner"></div>Working out what you need to do&hellip;</div>
    </div>`;
  container.appendChild(tasksCard);
  const tasksBody = tasksCard.querySelector("#admin-tasks-body");
  const tasksCountEl = tasksCard.querySelector("#admin-tasks-count");

  // ── Header / summary strip ────────────────────────────────────────────────
  const summary = el("div", { class: "card", style: { marginTop: "20px" } });
  summary.innerHTML = `
    <div class="card-header">
      <div>
  ...
  const state = {
    ...
    feedType: "all",
    feedSearch: "",
    // Per-admin task overrides: { [taskKey]: { dismissed:true } | { snoozeUntil: ms } }.
    // Persisted to Firestore (taskPrefs/{uid}) so they follow the admin across devices.
    taskOverrides: {},
    showSnoozed: false,     // toggle to reveal cleared/snoozed tasks
  };

  // Reference to this admin's task-prefs doc.
  const prefsRef = ctx.user?.uid ? doc(workflowDb, "taskPrefs", ctx.user.uid) : null;

  // Write a single override (dismiss / snooze / restore) and persist it.
  async function setTaskOverride(taskKey, override) {
    ...
    try {
      ...
      console.warn("[activity] failed to save task override", e);
  ...
  // Live-load this admin's dismiss/snooze choices.
  const unsubPrefs = prefsRef
    ? onSnapshot(prefsRef,
        (snap) => {
          state.taskOverrides = (snap.exists() && snap.data().overrides) || {};
          renderTasks();
        },
        (err) => { console.warn("[activity] task prefs snapshot error", err); },
      )
    : () => {};
  ...
  function renderTasks() {
    ...
    // Count chip reflects only what's actionable now.
    const urgent = active.filter((t) => t.priority === "urgent").length;
    tasksCountEl.innerHTML = active.length
      ? `<strong>${active.length}</strong> ${active.length === 1 ? "task" : "tasks"}${urgent ? ` · <span class="admin-tasks-count-urgent">${urgent} urgent</span>` : ""}`
      : "";
    ...
    if (!active.length) {
      tasksBody.appendChild(el("div", { class: "admin-tasks-clear", html: `
          ...
          <div>
            ...
            <div class="admin-tasks-clear-sub">No proposals to review, no unassigned editors, and nobody's overdue.${hidden.length ? ` ${hidden.length} task${hidden.length === 1 ? " is" : "s are"} cleared or snoozed.` : " Nicely done."}</div>
          </div>` }));
    } else {
      const list = el("div", { class: "admin-tasks-list" });
      for (const t of active) list.appendChild(renderTaskRow(t, false));
      tasksBody.appendChild(list);
    }

    // Footer: reveal / hide the cleared+snoozed tasks.
    if (hidden.length) {
  ...
  function renderTaskRow(t, isHidden) {
    const row = el("div", { class: `admin-task admin-task-${t.priority}${isHidden ? " admin-task-hidden" : ""}` });
    ...
    // When this row is in the cleared/snoozed section, show its status pill.
    ...
    // Manage menu (dismiss / snooze) for active rows; "Restore" for hidden rows.
    const manage = !t.key
      ? ""
      : isHidden
    ...
    if (t.message) {
      ...
      btn.addEventListener("click", async () => {
        try {
          ...
          ctx.toast(`Message for ${t.recipientName || "them"} copied — paste it into your texts.`, "success");
          setTimeout(() => {
            btn.classList.remove("admin-task-copy-done");
            label.textContent = "Copy text";
          }, 1800);
        } catch (e) {
          ctx.toast("Couldn't copy automatically. Message: " + t.message, "info");
        }
      });
    }

    // Restore (un-clear / un-snooze).
    const restoreBtn = row.querySelector('[data-act="restore"]');
    if (restoreBtn) {
      restoreBtn.addEventListener("click", () => {
        setTaskOverride(t.key, null);
        ctx.toast("Task restored.", "info");
      });
    }

    // Manage menu — open/close + snooze/dismiss actions.
    ...
    if (menuWrap) {
      ...
      trigger.addEventListener("click", (e) => {
        ...
        tasksBody.querySelectorAll(".admin-task-menu").forEach((m) => { m.hidden = true; });
...
// proposal", "Nudge Sam — overdue by 4 days". Each task carries a priority,
// an icon/color, an optional deep link, and a stable sort key so the most
// time-sensitive work floats to the top.
...
// Plain (un-escaped) first name for use inside a copy-to-clipboard text message,
// or null when there's no real person (e.g. "the writer" fallback). The message
// is plain text the admin pastes into their own texts, so it must NOT be escaped.
...
function craftMessage(kind, { name, title, idleDays, daysOverdue, daysUntilDue }) {
...
function buildAdminTasks(projects, users, overrides = {}) {
  const tasks = [];
  const now = Date.now();

  // Index active (non-reader) users for "who can I assign" hints.
  ...
  for (const project of projects) {
    ...
    if (tl[FINAL_STEP]) continue; // fully done — nothing to do
    ...
    if (project.proposalStatus !== "approved" && project.proposalStatus !== "rejected") {
      ...
      continue; // until approved, nothing else is actionable on this project
    }
  ...
  // Apply the admin's dismiss / snooze choices, splitting tasks into what's
  // currently actionable vs. what's been cleared or snoozed (still revealable).
  const active = [];
  const hidden = [];
  ...
  // Sort: priority bucket first (urgent > high > normal), then the per-task
  // sort score (which already folds in overdue distance + idle days).
...
// "back tomorrow" / "back in 4 days" for a snoozed task.
...
function ensureStyles() {
  ...
  s.textContent = `
    ...
    /* Restore button on cleared/snoozed rows */
    .admin-task-restore {
    ...
    /* State pills shown in the cleared/snoozed section */
    .admin-task-state {
      font-size:10px; font-weight:800; text-transform:uppercase; letter-spacing:.05em;
      padding:3px 8px; border-radius:999px;
    }
